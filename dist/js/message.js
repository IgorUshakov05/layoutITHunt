  ...
  ws.onmessage = (event) => {
    try {
      ...
      if (type === "message") {
        ...
        if (fromCurrentUser) {
          template = `
      <div data-id='${escapeHTML(result.id)}' class="boxMessage rigthMess">
        <span class="hisMessage myMessage">
          <span>${escapeHTML(result.content.toString())}</span>
          ...
          template = `
      <div data-id='${escapeHTML(result.id)}' class="boxMessage">
